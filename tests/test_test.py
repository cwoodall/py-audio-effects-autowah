...
def test_autowah_variable_cutoff_freq_response():
    ...
    omega_cs = np.linspace(0.1, np.pi, 10, endpoint=False)
    ...
    for omega_c in omega_cs:
        ...
        assert pytest.approx(0.0, abs=0.08) == np.abs((-6 - h_uut_dB) / 6)

        # Ideal Sinc style filter with a sharp roll-off
        h_ID = [
            (omega_c / np.pi) * np.sinc(omega_c * n / np.pi) for n in range(-N, N + 1)
        ]
        ...
        error = np.abs(
            (complex_to_magnitude_dB(h_uut) - complex_to_magnitude_dB(h))
        ) / complex_to_magnitude_dB(h)
        assert pytest.approx(0, abs=0.01) == error
...
def test_autowah_variable_cutoff_time_response():
    ...
    plt.plot(ts, ys)
    plt.savefig(
        "docs/test_results/test_autowah_variable_cutoff_time_response_variable_fc.png"
    )
...
def test_autowah_variable_cutoff_biquad_time_response():
    ...
    plt.plot(ts, ys)
    plt.savefig(
        "docs/test_results/test_autowah_variable_cutoff_biquad_time_response_variable_fc.png"
    )
    assert np.all(np.abs(ys) <= ys_max_values)
